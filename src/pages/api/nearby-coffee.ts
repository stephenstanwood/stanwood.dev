...
export const POST: APIRoute = async ({ request }) => {
  const { latitude, longitude, radius = 2000 } = await request.json();
  ...
  // Filter to operational places, compute distance, sort by nearest
  const results = places
    ...
    .filter((p: any) => p.isOpen !== false) // keep open or unknown
