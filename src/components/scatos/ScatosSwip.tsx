...
export default function ScatosSwip() {
  ...
  return <div className="sc-app" data-ready={Boolean(state)}>
    ...
    <div className="sc-layout"><aside className="sc-sidebar"><div className="sc-wish-title"><span className="sc-small-flower" aria-hidden="true">✳</span><h2>The wish list</h2></div><FiltersForm filters={filters} setFilters={changeFilters} count={eligible.length} />
      ...
      <section className="sc-main" aria-label="House browsing"><div className="sc-intro"><div><h1>{tab === 'browse' ? 'Maybe this is the one.' : tab === 'saved' ? 'A little collection of maybes.' : tab === 'matches' ? 'Your tastes have good taste.' : 'Not quite your kind of place.'}</h1><p>{tab === 'browse' ? 'Just looking is a very good place to start.' : tab === 'saved' ? 'Keep the houses—and the ideas—you love.' : tab === 'matches' ? 'The homes you’ve both quietly fallen for.' : 'Changed your mind? Every home gets another chance.'}</p></div>
